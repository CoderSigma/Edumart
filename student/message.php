...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['message'])) {
    ...
    if ($message) {
        ...
        $stmt->execute();
        header("Location: message.php?user_id=$chat_user_id" . ($item_id ? "&item_id=$item_id" : ""));
        exit();
    }
}
...
<head>
    ...
    <style>
        ...
        .message-form { display: flex; align-items: center; margin-top: 15px; }
        .message-input { flex-grow: 1; margin-right: 10px; }
        a.conversation-link { text-decoration: none; color: inherit; width: 100%; }
        a.conversation-link:hover { text-decoration: none; }
    </style>
</head>
<body class="bg-light">
            ...
            <ul class="list-group">
                <?php while ($conversation = $conversations->fetch_assoc()): ?>
                    <li class="list-group-item d-flex align-items-center conversation-item">
                        ...
                        <a href="message.php?user_id=<?= $conversation['user_id']; ?>&item_id=<?= $item_id ?? 0; ?>" class="conversation-link d-flex align-items-center">
                ...
                <div id="message-box" class="message-box">
                    <?php while ($msg = $messages->fetch_assoc()): ?>
                        <div class="message-item <?= $msg['sender_id'] == $user_id ? 'sent' : 'received'; ?>">
                            ...
                            <small class="d-block text-muted mt-1"><?= htmlspecialchars($msg['sent_at']); ?></small>
                        </div>
                    <?php endwhile; ?>
                </div>
